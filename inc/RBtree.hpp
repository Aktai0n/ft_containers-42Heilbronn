...
/*   Updated: 2022/02/15 22:57:51 by skienzle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
tree_iterator<Node_ptr,Value>::operator++()
{
	if (_current_node->_right != nullptr)
		_current_node = tree_min(_current_node->_right);
	else
	{
		while (!tree_is_left_child(_current_node))
...
tree_iterator<Node_ptr,Value>::operator--()
{
	if (_current_node->_left != nullptr)
		_current_node = tree_max(_current_node->_left);
	else
	{
		while (tree_is_left_child(_current_node))
			_current_node = _current_node->_parent;
...
private: // methods
	...
	void print_tree(const node_ptr node) const
	{
		if (node == nullptr)
			return;
		std:: cout << "key: " << node->_data.first << " value: " << node->_data.second << '\n';
...
RBtree<T,Compare,Alloc>::RBtree(const RBtree& other): 
	_size(), _root(this->_copy(other._root)), _comp(other._comp),
...
template<typename T, typename Compare, typename Alloc>
typename RBtree<T,Compare,Alloc>::node_ptr
RBtree<T,Compare,Alloc>::_copy(node_ptr node)
{
	if (node == nullptr)
		return nullptr;
